...
STOCKX_SEARCH_URL = "https://stockx.com/api/browse?&_search="
...
def get_URL_key(requested_product):
    try:
        URL = f"{STOCKX_SEARCH_URL}{requested_product['title']}"
...
def get_product_info(product):
    if product['URL_key'] is None:
        ...
        URL = f"{STOCKX_SEARCH_URL}{keyword}"
        source = requests.get(URL, headers=HEADERS)
...
def update_price(sheet, start_row, end_row):
    ...
    print(f"Total items: {total_items}\nTotal items that went up in price: {total_items_price_up}\nTotal items that went down in price: {total_items_price_down}\nTotal items that stayed the same in price: {(end_row - start_row + 1) - (total_items_price_down + total_items_price_up)}")
